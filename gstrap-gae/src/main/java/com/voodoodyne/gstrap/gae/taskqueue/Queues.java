...
public class Queues
{
	 ...
	 */
	public static <T extends Task> void add(final Collection<T> tasks) {
		log.debug("Enqueueing {} tasks", tasks.size());

		final Map<QueueHelper, List<T>> byQueue = tasks.stream().collect(Collectors.groupingBy(Task::defaultQueue));
	 ...
	 */
	public static <T extends Task> void add(final Collection<T> tasks, final long countdownMillis) {
